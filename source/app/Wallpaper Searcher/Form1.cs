...
namespace Wallpaper_Searcher
{
    public partial class home : Form
    {
        ...
        public static ProductData data = ReadJson(@"script\data.json");
        ...
        private void MainButton_Click(object sender, EventArgs e)
        {
            if (CheckActiveApp())
            {
                ...
                info.FileName = @"script\scraping.exe";
                info.WorkingDirectory = @"script\scraping.exe";
                Process.Start(info);
                Application.Exit();
            }
        }
        ...
        private void buttonRejected_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                if (CheckActiveApp())
                {
                    ...
                    ProcessStartInfo info = new ProcessStartInfo(@"script\scraping.exe");
                    Process.Start(info);
                    Application.Exit();
                }
            }
